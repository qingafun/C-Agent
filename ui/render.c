/*
 * ui/render.c — Dedicated terminal render thread for the AI Agent TUI.
 *
 * Owns all terminal redraws using a producer-consumer pattern: the main thread
 * and tool workers push events into a queue, and this thread drains them at
 * a fixed cadence. This prevents interleaved output when multiple threads
 * complete concurrently.
 *
 * Features:
 *   - Animated spinner while agent is "thinking"
 *   - Real-time per-tool status display (running/completed/failed)
 *   - Output sanitization to prevent ANSI escape injection
